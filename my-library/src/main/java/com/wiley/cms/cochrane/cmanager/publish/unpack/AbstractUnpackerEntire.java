 ...
 * @version 17.02.12
 */
public abstract class AbstractUnpackerEntire implements IPublish {
    private static final Logger LOG = Logger.getLogger(AbstractUnpackerEntire.class);
    ...
    private static final MessageFormat MSG_DETAILS =
            new MessageFormat("<br><b>Details</b>:<ul><li>{0} records have been updated.</ul>");
    ...
    protected AbstractUnpackerEntire(EntireDbWrapper db, String unpackName, String exportTypeName) {
        this.db = db;
    ...
    private void init(PublishWrapper publish) throws Exception {
        ...
        if (!publish.isNewPublish()) {
            ...
            packageFileName = publish.getFileName();
        } else {
            IPublishStorage ps = PublishStorageFactory.getFactory().getInstance();
            PublishEntity pe = ps.createPublishEntire(db.getDbName(), exportName, null, HWFreq.NONE.ordinal(), false);
            publish.setPublishEntity(pe);
            //PublishEntity pe = rs.findEntirePublish(db.getDbName(), exportName);
            exportId = pe.getId();
            packageFileName = pe.getFileName();
        }
    ...
    public void start(PublishWrapper publish, PublishOperation op) {
        ...
        try {
            ...
            if (!checkPublishReadiness()) {
                rs.setUnpacking(exportId, false, true);
        ...
        rs.updatePublishUnpacking(exportId, false, new Date(), true);
    ...
    protected String getReport(boolean includeIssue) {
        ...
        if (includeIssue) {
            map.put(ISSUE_AND_DB_NAME_NOTIF_PARAM, getIssueAndDb("unpacking.db_name_entire"));
        } else {
    ...
    private void addActivityLogOnSuccess() {
        Object[] args = new Object[]{getPackageFileName(), db.getDbName()};
        String msg = MSG_UNPACK_SUCCESS.format(args);

        msg = msg.concat(getDetailsMessage());

        DatabaseEntity database = rs.getDatabaseEntity(db.getDbName());
        als.info(ActivityLogEntity.EntityLevel.DB, ILogEvent.PUBLISH_UNPACKING_SUCCESS,
                database.getId(), db.getDbName(), PUBLISH_UNPACKER, msg);
    }

    protected String getDetailsMessage() {
        Object[] args = new Object[]{getUnpackedRecordsCount()};
        return MSG_DETAILS.format(args);
    }

    private void activityLogOnFailed(String message) {
        Object[] args = new Object[]{getPackageFileName(), db.getDbName(), message};
        String msg = MSG_UNPACK_FAIL.format(args);
        DatabaseEntity database = rs.getDatabaseEntity(db.getDbName());
        als.error(ActivityLogEntity.EntityLevel.DB, ILogEvent.PUBLISH_UNPACKING_FAILED, database.getId(),
                db.getDbName(), PUBLISH_UNPACKER, msg);
    }
    ...
    private void sendNotificationOnFailed(String message) {
        ...
        map2.put(ISSUE_AND_DB_NAME_NOTIF_PARAM, getIssueAndDb("unpacking.db_name_entire_failed"));
        ...
        map.put(DATABASE_NOTIF_PARAM, db.getDbName());
    ...
    protected Integer countUnpackedRecords() {
        Object[] args = new Object[]{FileNameMatcher.valueOf(getDb().getDbName().toUpperCase()).getCommand(),
